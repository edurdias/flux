...
def test_task_authorization_error_includes_namespace():
    ...
    err = TaskAuthorizationError(
        ...
        task_id="tid",
        subject="alice",
        required_permission="workflow:billing:invoice:task:load:execute",
    )
    assert err.required_permission == "workflow:billing:invoice:task:load:execute"
    assert "billing" in err.required_permission
