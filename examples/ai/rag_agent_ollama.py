...
@workflow.with_options(name="rag_agent_ollama_simple")
async def rag_agent_ollama_simple(ctx: ExecutionContext[dict[str, Any]]):
    """
    Simple all-in-one RAG workflow (indexes on every query).

    This workflow is simpler but less efficient - it re-indexes documents on every query.
    For production use, use rag_index_documents + rag_query_documents instead.
...
# Alternative workflow with pause/resume (for demonstration - not recommended for large data)
@workflow.with_options(name="rag_agent_ollama_pausable")
async def rag_agent_ollama_pausable(ctx: ExecutionContext[dict[str, Any]]):
    """
    RAG workflow with pause/resume capability.

    NOTE: This version uses pause/resume but may have issues with large document sets
    due to state serialization. Use rag_agent_ollama for production.
    ...
    Mode 2 - Query (resume after indexing):
    {
        "mode": "query",
    ...
    """
    input_data = ctx.input or {}
    mode = input_data.get("mode", "index")
    ...
    if mode == "index":
        # Index mode: Load and index documents
        docs_path = input_data.get("docs_path")
        if not docs_path:
            return {
                "error": "Missing 'docs_path' parameter for index mode",
                "execution_id": ctx.execution_id,
            }
        ...
        # Pause and wait for query
        # Due to Flux's deterministic replay, chunks and index_bytes will be available after resume
        await pause(
            "ready_for_queries",
            output={
                "status": "indexed",
                ...
                "embedding_model": embedding_model,
                "execution_id": ctx.execution_id,
                "message": "Documents indexed successfully. Resume with a query to ask questions.",
            },
        )

        # After pause, get query from resume input
        query = input_data.get("query")
        if not query:
            return {
                "error": "No query provided in resume input",
                "execution_id": ctx.execution_id,
            }
        ...
        # Retrieve relevant chunks (chunks and index_bytes are still in scope)
        relevant_chunks = await retrieve_relevant_chunks(
            query,
            index_bytes,
            chunks,
        ...
        return {
            "error": "Invalid mode. Use 'index' to build the knowledge base first.",
            "execution_id": ctx.execution_id,
        }
