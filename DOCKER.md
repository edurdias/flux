...
- **Upgrading from a root-based image**: named volumes created by older (root-running) images may contain root-owned files (e.g. `flux-data` mounted at `/app/.flux`). After upgrading, fix ownership with `docker run --rm -u 0 -v flux-data:/data <image> chown -R 1000:1000 /data`, or recreate the volume (`docker volume rm flux-data`) if its contents are disposable.
