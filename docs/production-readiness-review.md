...
15. `call()`/`workflow_agent()` transient passthrough; sticky same-worker in-process execution (§6, phase 2, **done** — in-process `call()` for transient workflow objects, ~2.3 ms/hop measured; server-side sticky routing shipped as an `X-Flux-Preferred-Worker` hint stored on the execution and honored by batch dispatch when the worker is eligible).
...
- **2026-07-15 — Dynamic workflows: ephemeral agent registration (series PR 2).** Agents author workflow source at runtime; dynamic workflows are ordinary workflows (same catalog/dispatch/replay/approval machinery) reached through a policy-enforcing registration path. Agent-only by construction: `POST /workflows/dynamic` accepts nothing but execution tokens when auth is on, and the per-principal namespace (`dyn-<slug>-<hash8>`) is derived server-side from the token subject — cross-namespace writes are impossible by construction, with `workflow:<ns>:*:register` still required on top (default-deny). The `dyn-` prefix is reserved: the shared namespace validator rejects it everywhere (decorators, uploads, inline auto-registration), so nothing un-stamped can squat an agent namespace — the enforcement twin of the server stamping `require_runner` (default `docker-airgapped`) into every dynamic entry's metadata. Policy is a with_options allowlist (`name`, `durability`, `secret_requests`, `output_storage`); schedules, services, requests, affinity, routing, namespace, and runner are structured-error rejections — a one-shot authoring call must not install standing schedules or public endpoints. Source is parsed with the catalog's exec-free static parser only (hostile source never runs on the server; the input-schema enrichment that requires import is skipped). Idempotent by source hash — byte-identical re-registration returns the existing entry, keeping replay and the source-hash-keyed module cache coherent. GC rides the retention job (last-used TTL, default 7d, never under a live execution). Agent-facing `create_workflow`/`run_workflow` ship as flux tasks and a `dynamic_workflows` tool group; they authenticate with the calling execution's own token. Deferred: auto-provisioning grants from an agents-table flag (agents lack service principals today), the vocabulary layer (PR 3).
- **2026-07-14 — docker-airgapped runner: isolation as a dispatch-visible guarantee.** First PR of the dynamic-workflows series (spec: `docs/specs/2026-07-14-airgapped-runner-spec.md`); standalone value for SEC4 (pin untrusted registered workflows). A distinct runner name — not a flag — so the hardened profile is worker-side configuration and dispatch matching gives placement for free. The profile is code, not config (`_locked_args()`, emitted after operator `extra_args` so docker's last-wins parsing structurally favors it): `--network=none`, read-only rootfs + size-capped tmpfs `/tmp`, `--cap-drop=ALL`, `no-new-privileges`, pids/memory/cpu limits — required non-empty, unlike the base docker runner's "empty = unlimited". `extra_args` are veto-listed at worker startup (denylist over allowlist: benign knobs can't be enumerated, the guarantees to protect can). The wall-clock ceiling landed as a **generic `execution_timeout` on `SubprocessRunner`** (subprocess and plain docker gain it too; airgapped defaults it to 900s), and maps to **terminal FAILED for both durabilities** via a new `ExecutionTimedOut` — deliberately not the crash path, whose durable-release would re-dispatch a deterministic timeout forever. Everything else — child protocol, credential sanitization, relays, cancellation, crash mapping — inherited unchanged.
- **2026-07-12 — Replay-determinism contract documented (D6).** New user-facing page (`docs/core-concepts/replay-determinism.md`, in the mkdocs nav): the event-sourced replay model, the contract ("everything outside @task calls re-runs on every replay and must reach the same task calls in the same order with the same arguments"), what breaks it (wall clock, randomness, non-task I/O, environment-dependent branching) with the silent-divergence failure mode spelled out, the deterministic built-ins (`now()`, `uuid4()`, `randint()`, `choice()`, replay-aware `sleep()`), the runtime's own guarantees (content-addressed per-call identity with occurrence counters, retry-attempt tracking, logged failure paths, cache-vs-replay separation), and a pre-flight self-audit checklist. A debug-mode replay-divergence detector remains future work.
- **2026-07-12 — One-time worker join tokens; global request-body cap.** SEC3's remainder and SEC5's body-limit item. Registration now accepts a second credential class: single-use join tokens minted per worker (`flux server join-token` / `POST /admin/workers/join-tokens`, `admin:workers:manage`), stored as SHA-256 hashes with a TTL (default 1h) and consumed by one atomic UPDATE so a race cannot double-spend one; used rows are kept inside a 24h grace window as a join audit trail. The shared bootstrap token still works by default (`[flux.workers] bootstrap_token_enabled = true`) so fleets migrate at their own pace, then turn the master secret off. Separately, a pure-ASGI body-size middleware (`server_max_body_size`, default 64 MiB, 0 disables) rejects oversized bodies with 413 — up front when Content-Length declares it, mid-stream via a counting receive wrapper when chunked. It is registered innermost deliberately: the streamed-body 413 is raised from `receive()` during body parsing, and a BaseHTTPMiddleware between it and the router (SlowAPI, the anonymous-policy hook) would launder that HTTPException into a generic 400 through its receive bridge.
- **2026-07-12 — Status reads skip event hydration; listings gained opt-in pagination.** `ContextManager.get_summary` serves the `GET /executions/{id}` summary path (and its workflow-read check) from the execution row alone — no event loading or unpickling — with one targeted event lookup only for the paused-payload case; `last_event_ordinal` gives the sync-wait/stream loop a cheap change signal so it re-hydrates the full context only when the persisted log actually advanced (D5). `GET /workers`, `/admin/roles`, and `/admin/principals` accept optional `limit`/`offset` (unpaginated calls keep the full-list contract); the principals route slices before its per-principal role lookups. S3's least-loaded aggregate was left as-is: the composite `(state, worker_name)` index makes it an index-only scan, and the event dispatcher already bypasses it for fleets.
...
- **2026-07-04 — Self-health is advisory and heartbeat-carried; sticky routing is a hint.** Workers self-report event-loop starvation on their existing heartbeat pong (no new channel); the server excludes them from dispatch but never evicts on self-report — running work finishes, and the reaper stays the authority for dead workers. Relayed `call()`s carry the caller's worker name; dispatch prefers it only when eligible, so the hint can never strand or misroute work (poll mode ignores it).
- **2026-07-08 — Dynamic routing: declarative scoring evaluated server-natively; arbitrary logic lives on workers.** Of the evaluation-site options considered (server-native DSL, worker-advertised metrics, submit-site route function, sandboxed server-side evaluator, router-as-workflow, external endpoint), v1 ships the first two: workflows declare a weighted scoring policy (`routing=score(prefer/least/most/sticky)`, `input()` refs for payload locality) that the catalog extracts statically (AST — an unparseable policy fails registration loudly) and the event dispatcher evaluates natively over the eligible set; arbitrary user logic runs where user code already runs — the worker — published as heartbeat-carried metrics (`metrics_provider`, bounded dict[str, float]) that policies rank via `metric:*` selectors. A policy owns its score stage (the sticky hint participates only via an explicit `sticky()` term); malformed policies degrade to least-loaded; poll mode ignores policies. Metrics persistence is change-gated so DB writes follow the metrics-refresh cadence, not the heartbeat rate. Built-in `flux.*` metrics ship by default (loop lag p95, failure/crash rates, throughput, duration p95, startup overhead, utilization) — computed worker-side over fixed windows, published as scalars; the reserved prefix is stripped from provider output so user values cannot impersonate built-in signals. Deferred: submit-site `route=` callable on `call()`, sandboxed evaluator, router-as-workflow (reframed as an async placement advisor writing hints).
- **2026-07-11 — Authorization: scheduling requires the caller to be runnable-authorized; execution reads are workflow-scoped.** Two escalation/leak guards. (1) `schedule:*:manage` alone no longer lets a caller bind a workflow to a service account — at trigger time the workflow runs with the SA's roles, so create (and SA rebinds on update) now require the *caller* to pass the same `authorize()` check the trigger applies to the SA. A caller can only schedule work they could run themselves; finer SA-impersonation policy (e.g. `principal:*:impersonate`) is deferred. (2) The flat `execution:*:read` grant no longer crosses workflow read boundaries: `GET /executions/{id}` verifies `workflow:{ns}:{name}:read` on the execution's workflow, and the flat listing scopes results per readable workflow using the approvals-listing pattern (authorize per distinct workflow before the paginated query; `auth_filtered` flag on the response). Broad readers (`workflow:*:*:read`) are unaffected.
- **2026-07-11 — Replay determinism: per-call occurrence ids; terminal events on every failure path; lifecycle flags track workflow events.** Four related engine fixes. (1) Repeated identical task calls previously collapsed — the replay short-circuit matched on the argument-derived task_id alone, so `await send_email(x)` twice ran the body once. Each call now consumes a per-run occurrence counter (runtime-only, derived deterministically from program order; never serialized) and repeats get `~{n}`-suffixed ids; the first occurrence keeps the bare id so existing event logs replay unchanged, and `cache=True` keeps memoizing via the bare cache key. (2) A failing fallback or rollback now records a terminal TASK_FAILED (storing the exact propagated exception) — without it, replay re-executed the body and fallback. (3) Replaying a TASK_FAILED value that crossed a JSON checkpoint hop (worker→server degrades exceptions to `{"type","message"}`) reconstructs the recorded exception type (ExecutionError / ApprovalRejected) instead of crashing with `raise <dict>` → TypeError; full value fidelity across the wire (e.g. datetime outputs replaying as ISO strings) is deferred to a wire-format rev with protocol negotiation. (4) `has_finished`/`is_paused`/`is_cancelled` now key off the most recent WORKFLOW_* lifecycle event instead of `events[-1]`, so a `parallel()` sibling completing after WORKFLOW_FAILED/PAUSED (gather does not cancel siblings) cannot flip a finished or paused execution back to "running".
- **2026-07-10 — Standing approvals ride the approval row, not a new event type (issue #74).** An approval decided with `--always` stores `scope="execution"` on the existing `approval_requests` row; later gates on the same task *name* within that execution find the grant at registration time and auto-approve without pausing, each materializing an `approved` audit row (approver copied from the grant, `reason="standing grant"`). Chosen over the issue's original event-based design because grant resolution inside `register` needs zero gate changes across all three approval transports (inline store, worker endpoints, runner-child pipe) and keeps the row as the single durable audit record. Name-scoped matching deliberately covers retry-attempt gates (`…~retryN` call ids). Constraints: grants never cross executions, `scope="execution"` is invalid on rejections, no standing reject, and revocation is deferred — an operator who needs per-call control simply doesn't use `--always`.
